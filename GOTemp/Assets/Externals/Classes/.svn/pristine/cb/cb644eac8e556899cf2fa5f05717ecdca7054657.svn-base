...
namespace GambitUtils
{
    ...
    public struct Maths
    {
        ...
        {
            bounds = new Bounds(transform.TransformPoint(bounds.center), bounds.size);
            return bounds;
        }

        public static Bounds MergeBounds(Bounds result, Transform transform, Bounds added)
        {
            added = TransformBounds(transform, added);
            result.SetMinMax(Mathv.Min(result.min, added.min), Mathv.Max(result.max, added.max));
            return result;
        }
    }
}